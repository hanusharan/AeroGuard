"""FINAL v0.3 calibration gate (ONE narrowly-scoped test): does capping
the combined two-pulse duration (aeroguard_dataset/
control_profiles_candidate_d_v3.py) reduce the gamma-termination rate
among non-crossing trajectories introduced by v2's same-sign fix, while
preserving v2's elimination of dive-then-zoom-climb crossings?

Generates ~175 trajectories (matching the v2/reconciliation baseline's n
and seed for comparability) using ONLY Candidate D v3 -- no new candidate
...
from aeroguard_dataset.control_profiles_candidate_d_v3 import build_candidate_d_v3_profile
...
def generate_batch(n: int, seed: int):
    ...
    for i in range(n):
        ...
        profile = build_candidate_d_v3_profile(rng, alpha_trim, throttle_trim, elevator_trim, DURATION_S)
        ...
        metadata_rows.append({
            "trajectory_id": tid, "generation_mode": "gradual_D_two_stage_v3",
...
def main():
    print(f"Generating Candidate D v3: n={N_TRAJECTORIES}, seed={SEED}, dt={DT}, duration={DURATION_S}s")
    ...
    raw_df.to_parquet(os.path.join(OUT_DIR, "candidate_d_v3_raw.parquet"), index=False)
    meta_df.to_csv(os.path.join(OUT_DIR, "candidate_d_v3_metadata.csv"), index=False)
    ...
    print(f"(baseline Candidate D v2: crossed=22.9% gamma_term=57.7% completed=41.1%)")
    ...
    cross_df.to_csv(os.path.join(OUT_DIR, "candidate_d_v3_crossing_classification.csv"), index=False)
    ...
    pd.DataFrame([summary]).to_json(os.path.join(OUT_DIR, "candidate_d_v3_summary.json"), orient="records", indent=2)
    print(f"\nWrote summary to {os.path.join(OUT_DIR, 'candidate_d_v3_summary.json')}")
    ...
    fig.suptitle("Candidate D v3: representative crossings (same-sign, zero-gap fix)")
    fig.tight_layout()
    fig.savefig(os.path.join(OUT_DIR, "plots", "05_candidate_d_v3_traces.png"), dpi=110)
    plt.close(fig)
